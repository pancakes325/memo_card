...
ResultGroupBox = QGroupBox('Итог')
lb_test_result = QLabel('')
layout_test_res = QVBoxLayout()
layout_test_res.addWidget(lb_test_result, alignment=Qt.AlignCenter)
...
def show_test_result():
    global points, total
    lb_test_result.setText(str(points)+" из "+str(total))
    RadioGroupBox.hide()
    AnsGroupBox.hide()
    ResultGroupBox.show()
...
def next_question():
    global cur_question, points, total
    cur_question += 1
...
questions = [
    Question('Выбери перевод слова "переменная"', 'variable', 'variation', 'variant', 'changing'),
    Question('Выбери статический язык программирования', 'Java', 'Python', 'JavaScript', 'PHP'),
    Question('Самый популярный язык программирования', 'Python', 'Java', 'C#', 'C++')
]
next_question()
